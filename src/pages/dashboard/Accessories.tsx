...
import { ShoppingCart, Star, Zap, CreditCard, Smartphone, Tag, ChevronDown, ArrowLeft, Building2, User, Info, Image, X } from "lucide-react";
  ...
  return <div className="space-y-6">
      ...
      {showCustomAlert && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-amber-50 border-2 border-amber-800 rounded-lg p-6 max-w-md mx-4 relative animate-slide-in-right shadow-xl">
            <button
              onClick={() => setShowCustomAlert(false)}
              className="absolute top-3 right-3 text-amber-800 hover:text-amber-900 transition-colors"
            >
              <X className="h-5 w-5" />
            </button>
            <div className="text-amber-800 text-center">
              <h3 className="text-lg font-semibold mb-2">Custom NFC Cards coming soon!</h3>
              <p className="text-sm">We're working on bringing you custom NFC card options. Stay tuned!</p>
            </div>
          </div>
        </div>
      )}
